...
exports.handler = async function (event) {
  try {
    if (event.httpMethod === "GET" && event.queryStringParameters?.action === "qr") {
      return qrResponse(event.queryStringParameters.url);
    }
    ...
    if (event.httpMethod === "GET") {
      return readSession(store, event.queryStringParameters || {});
    }
...
async function qrResponse(url) {
  const safeUrl = String(url || "").slice(0, 1200);
